(() => {
  ...
  // src/ts/modules/swiper/swiper.ts
  core_default.use([Navigation, Pagination, Autoplay, Thumb, EffectFade]);
  var swiper = () => {
    const slider = document.querySelectorAll(".js-swiper");
    for (let i = 0; i < slider.length; i++) {
      const num = ("00" + (i + 1)).slice(-2);
      slider[i].className += num;
      const swiper2 = new core_default(".js-swiper" + num, {
        spaceBetween: 30,
        effect: "fade",
        speed: 2e3,
        navigation: {
  ...
  // src/ts/modules/swiper/setSwiper.ts
  var setSwiper = () => {
    swiper();
  };
  ...
  // src/ts/index.ts
  var switchLink = () => {
    const target = (0, import_jquery.default)(".js-switchlink");
    if (!target.length)
      return;
    target.each((i, e) => {
      const splink = (0, import_jquery.default)(e).data("splink");
      if (isSumaho()) {
        console.log(splink);
      } else {
        console.log("pc");
      ...
      (0, import_jquery.default)(e).attr("href", `${splink}`);
    });
  };
  switchLink();
  test();
